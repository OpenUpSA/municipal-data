...
class ScorecardNoDataTest(BaseSeleniumTestCase):
    fixtures = ["scorecard-no-data", "seeddata"]

    def setUp(self):
        super(ScorecardNoDataTest, self).setUp()
        Site.objects.filter(id=2).update(
            domain="municipalmoney.org.za", name="Scorecard"
        )

    @override_config(CAPITAL_PROJECT_SUMMARY_YEAR="2022/2023")
    def test_comparison_drowndown_no_data(self):
        geography = Geography.objects.get(geo_code="BUF")
        utils.load_file(geography, mock_project_next_financial_year(), "2022/2023")
