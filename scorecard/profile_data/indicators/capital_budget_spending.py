...
class CapitalBudgetSpending(IndicatorCalculator):
    ...
    def get_muni_specifics(cls, api_data):
        ...
        populate_periods(
            periods,
            group_assets_by_year(results["capital_expenditure_budget_v1"]),
            "budget",
        )
        populate_periods(
            periods,
            group_assets_by_year(results["capital_expenditure_actual_v1"]),
            "actual",
        )
        # Populate periods with v2 data
        populate_periods(
            periods,
            group_amount_by_year(results["capital_expenditure_budget_v2"]),
            "budget",
        )
        populate_periods(
            periods,
            group_amount_by_year(results["capital_expenditure_actual_v2"]),
            "actual",
        )
